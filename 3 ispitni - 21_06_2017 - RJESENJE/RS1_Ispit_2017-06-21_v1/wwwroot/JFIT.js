...
function DodajAjaxEvente() {
    ...
    $("form[ajax-poziv='daa']").submit(function (event) {
        ...
        var stavkaID = $(this).attr("stavkaID");                // uzme se stavkaID
        var bodovi = document.getElementById(stavkaID).value;       // vrijednost bodova
        alert(bodovi);

        urlZaPoziv = urlZaPoziv + + "?bodovi=" + bodovi + "&stavkaID=" + stavkaID;      // sastavi se url 

        $.get(urlZaPoziv, function (rezultat, status) {

            $("#" + divZaRezultat).html(rezultat);
        });
    });
