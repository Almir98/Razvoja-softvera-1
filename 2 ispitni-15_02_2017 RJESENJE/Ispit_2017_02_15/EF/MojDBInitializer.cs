...
namespace Ispit_2017_02_15.EF
{
    public class MojDBInitializer
    {
        ...
        public static void Izvrsi(MojContext context)
        {
            ...
            for (int i = 0; i < 25; i++)
            {
                var cas = new OdrzaniCas { AngazovanId = 1, Datum = DateTime.Now };
            ...
            for (int i = 0; i < 40; i++)
            {
                var detalji = new OdrzaniCasDetalji { OdrzaniCasId = random.Next(1, 25), BodoviNaCasu = new Random().Next(0, 100), Prisutan = true, SlusaPredmetId = new Random().Next(1, 4) };
