...
namespace Ispit.Data
{
    public class MojDBInitializer
    {
        public static void Izvrsi(MojContext _context)
        {
            ...
            for (int i = 1; i <= 4; i++)
            {
                odjeljenja.Add(new Odjeljenje() { SkolskaGodina = "2015/16", Oznaka = i + "-a", Razred = i, Nastavnik = nastavniks[i % 20] });
                odjeljenja.Add(new Odjeljenje() { SkolskaGodina = "2015/16", Oznaka = i + "-b", Razred = i, Nastavnik = nastavniks[i % 20] });
            }
            int b = 0;
